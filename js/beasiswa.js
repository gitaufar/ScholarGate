...
const form = document.getElementById("tambah_beasiswa_form");
...
async function fetchBeasiswa() {
  ...
  listBeasiswa.forEach((element) => {
    ...
    newRow.innerHTML = `
        <td class="id_beasiswa">${element.id_beasiswa}</td>
...
form.addEventListener('submit', async function (event) {
    ...
    const beasiswa = {
        ...
        jenis_beasiswa: document.getElementById("jenis").value,
        ipk_minimal: document.getElementById("ipk_minimal").value,
        kriteria_status_ekonomi: document.getElementById("kriteria").value,
        jumlah_beasiswa: parseFloat(document.getElementById("jumlah_beasiswa").value),
        tenggat_waktu: document.getElementById("tenggat_waktu").value,
        deskripsi: document.getElementById("deskripsi").value,
        sumber_beasiswa: document.getElementById("sumber_beasiswa").value
    };

    console.log("Data Beasiswa:", beasiswa);

    try {
        const result = await addBeasiswa(beasiswa);
        if (!result) {
            alert("Gagal menambahkan data.");
            return;
        }

        const beasiswa_baru = await getBeasiswa();
        if (!beasiswa_baru || beasiswa_baru.length === 0) {
            alert("Tidak ada data beasiswa yang ditemukan.");
            return;
        }
        ...
        const newRow = document.createElement("tr");
        const lastBeasiswa = beasiswa_baru[beasiswa_baru.length - 1];
        newRow.innerHTML = `
            <td class="id_beasiswa">${lastBeasiswa.id_beasiswa}</td>
        <td>${lastBeasiswa.nama_beasiswa}</td>
        <td>${lastBeasiswa.jenis_beasiswa}</td>
        <td>${lastBeasiswa.ipk_minimal}</td>
        <td>${lastBeasiswa.kriteria_status_ekonomi}</td>
        <td>${lastBeasiswa.jumlah_beasiswa}</td>
        <td>${lastBeasiswa.tenggat_waktu}</td>
        <td>${lastBeasiswa.deskripsi}</td>
        <td>${lastBeasiswa.sumber_beasiswa}</td>
            <td><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 13" fill="none">
  <path d="M0 10.352V13H2.64802L10.4579 5.19011L7.80989 2.5421L0 10.352ZM13 2.64802L10.352 0L8.56545 1.79359L11.2135 4.44161L13 2.64802Z" fill="#193A6F"/>
</svg></td>
        <td><svg class="delete_row" xmlns="http://www.w3.org/2000/svg" width="15" height="16" viewBox="0 0 15 16" fill="none">
